...
#     label = eng_label_encoder.inverse_transform([pred_class])[0]
#     label_map = {'0': 'Fix Bug', '1': 'Feature Request', '2': 'Non-Informatif'}
#     prediction_text = label_map.get(label, label)
...
#         prediction_text = predict_text_english(text)
...
#         cursorr.execute('INSERT INTO predict_text(review_text,class,history_id) VALUES(%s,%s,%s)', (text,prediction_text,historis_id,))
#         mysql.connection.commit()
...
#         logits = model_eng(encodings['input_ids'], encodings['attention_mask'])
#     y_pred = torch.argmax(logits, dim=1).numpy()
#     label_names = eng_label_encoder.inverse_transform(y_pred)
#     merged_data['predicted_label'] = label_names

#     # Urutkan dan hitung jumlah label
#     label_categories = list(eng_label_encoder.classes_)
...
#         prediction = label_encoder_indo.inverse_transform(new_data_pred_classes)
...
#         logits = model_multilanguage(encodings['input_ids'], encodings['attention_mask'])
#     y_pred = torch.argmax(logits, dim=1).numpy()
#     label_names = multilanguage_label_encoder.inverse_transform(y_pred)
#     merged_data['predicted_label'] = label_names

#     # Urutkan dan hitung jumlah label
#     label_categories = list(multilanguage_label_encoder.classes_)
#     merged_data['predicted_label'] = pd.Categorical(merged_data['predicted_label'], categories=label_categories, ordered=True)
#     merged_data = merged_data.sort_values(by='predicted_label')
...
#     pred_class = torch.argmax(logits, dim=1).item()
#     label = multilanguage_label_encoder.inverse_transform([pred_class])[0]
#     return label
...
#         prediction_text = predict_text_multilanguage(text)
